...
def test__wepy_h5_traj_init(_wepy_h5_traj_init):
    ...
    with WepyHDF5(_wepy_h5_traj_init, mode="r") as wepy_h5:
        ...
        for traj_idx in (0, 1):
            ...
            assert wepy_h5.traj_field_entity(0, traj_idx, "positions").shape == (
                1,
                2,
                3,
            )
            assert wepy_h5.traj_field_entity(0, traj_idx, "box_vectors").shape == (
                1,
                3,
                3,
            )
            assert wepy_h5.traj_field_entity(0, traj_idx, "kinetic_energy").shape == (
                1,
                1,
            )

            assert "data" in wepy_h5.traj_field_entity(0, traj_idx, "velocities")
            assert "_sparse_idxs" in wepy_h5.traj_field_entity(
                0, traj_idx, "velocities"
            )

            assert wepy_h5.traj_field_entity(0, traj_idx, "velocities")[
                "data"
            ].shape == (0, 0, 0)
            assert wepy_h5.traj_field_entity(0, traj_idx, "velocities")[
                "data"
