...
class OpenMMRayTask:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        logging.getLogger("OpenMMRayTask").info(
            "Configured logging in OpenMMRayTask process"
        )
...
class OpenMMRayPoolWorkMapper:
    ...
    def __init__(
        ...
        if device_platform_properties is not None:
            ...
            if len(device_platform_properties) != device_ids:
                raise ValueError(
                    f"{len(device_ids)} requested, but only {len(device_platform_properties)} given."
                )
    ...
    def init(
        ...
        self._ray_ctx = ray.init(
            **(self._ray_init_args if self._ray_init_args is not None else {})
        )
    ...
    def map(
        ...
        logger.info(
            f"Running map on {len(walker_states)} in batches of {self._num_workers}"
        )
            ...
            for batch_idx, batch in enumerate(
                ...
                for batch_task_idx, (walker_state, task) in enumerate(batch):
                    ...
                    if self._device_ids is not None and self._platform in GPU_PLATFORMS:
                        logger.info(
                            "Worker platform configured and resolving platform properties."
                        )
                    ...
                    logger.info(
                        f"Injecting: platform={self._platform}, platform_kwargs={platform_kwargs}"
                    )
                    result = pool.apply_async(
                        _task,
                        args=(walker_state,),
                        kwargs=dict(
                            platform=self._platform, platform_kwargs=platform_kwargs
                        ),
                    )
